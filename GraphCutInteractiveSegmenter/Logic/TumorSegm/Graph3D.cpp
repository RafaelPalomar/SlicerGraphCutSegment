...
void Graph3D::addSmoothCost(const Data3D<double>& cap, const Data3D<double>& rev_cap, const MyBasic::Index3D& shift)
{
...
{
  int num = id1.size();
  std::cout << "Begin addSmoothCost!" << std::endl;
  for (int i = 0; i < num; i++)
  {
    //			cout<<i<<endl;
    add_edge(id1[i], id2[i], cap[i], rev_cap[i]);
  }
  std::cout << "Done addSmoothCost!" << std::endl;
}
...
Data3D<LABEL> Graph3D::get_labeling()// const
{
  ...
  for (int s = 0; s < nSli; s++)
  {
    ...
    for (int r = 0; r < nRow; r++)
    {
      for (int c = 0; c < nCol; c++)
      {
        label.at(s, r, c) = what_segment(MyBasic::Index3D(s, r, c));
      }
    }
  }
  return label;
...
LABEL Graph3D::what_segment(const MyBasic::Index3D& i)// const
{
