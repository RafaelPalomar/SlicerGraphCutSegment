...
namespace ImageProcessing
{
  ...
  Data2D<intensityType> index2intensity(const Data2D<indexType>& index, const std::vector<intensityType>& map);
  std::vector<double> hist(const Data2D<int>& bin_id, const Data2D<LABEL>& mask, int bin_num, int indicator = 1);
  std::vector<double> hist(const Data3D<int>& bin_id, const Data3D<LABEL>& mask, int bin_num, int indicator = 1);
  ////////////////////////////////////////////////////////////
  //           translation mode
  ...
  */
  ...
  template<typename T>
  std::vector<MyBasic::Index2D> boundaryPoints(const Data2D<T>& bw, uchar connectivity);
  template<typename T>
...
template<typename indexType, typename intensityType>
Data2D<intensityType> ImageProcessing::index2intensity(const Data2D<indexType> & index, const std::vector<intensityType> & map)
{
...
template<typename T>
std::vector<MyBasic::Index2D> ImageProcessing::boundaryPoints(const Data2D<T> & bw, uchar connectivity)
{
  std::vector<MyBasic::Index2D> bdpoint;
...
{
  std::vector<MyBasic::Index2D> bdpoint = boundaryPoints(bw, connectivity);
  bdpoint = bdpoint - center;
  std::vector<double> dist(bdpoint.size());
