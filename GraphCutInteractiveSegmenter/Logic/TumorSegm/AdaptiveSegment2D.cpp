...
void AdaptiveSegment2D::computeDataterm()
{
  ...
  std::vector<double> fghist = IP::hist(bin_id, mask, cfg.bin_num, 1);
  double sm = sum(fghist);
  std::vector<double> fgprob = fghist / sum(fghist);
  std::vector<double> bkhist = IP::hist(bin_id, mask, cfg.bin_num, 0);
  std::vector<double> bkprob = bkhist / sum(bkhist);
  Data2D<int> bin_id_roi = bin_id; //bin_id(seg_region);

  std::vector<double> fglikelihood = -log(fgprob + SMALLCONST);
  std::vector<double> bklikelihood = -log(bkprob + SMALLCONST);
...
void AdaptiveSegment2D::hardConstraintStarSeeds()
{
  ...
  std::vector<MyBasic::Index2D> points = lliu::getPointsWithinCircle(starcenter, cfg.seed_radius);
...
{
  std::vector<MyBasic::Index2D> points = lliu::getBorderPoints(img_roi.getSize(), 1);
