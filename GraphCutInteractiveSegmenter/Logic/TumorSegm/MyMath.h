...
template<typename T>
double standardDeviation(const std::vector<T>& a);
...
template<typename T>
std::vector<double> log(const std::vector<T>& op);

template<typename T>
std::vector<T> floor(const std::vector<T>& op);
...
template<typename T>
void divide(std::vector<T>& data, T dividor);
...
std::vector<double> log(const std::vector<T>& op)
{
  std::vector<double> result(op.size());
  int s = op.size();
