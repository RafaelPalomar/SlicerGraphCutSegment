...
public:
  ...
  {
    set(MyBasic::Range3D(range_sli, range_row, range_col), val);
  }

  void set(const MyBasic::Range3D & range, T val)
  {
  ...
  void setAll(T val)
  {
    if (!isEmpty()) set(MyBasic::Range3D(Range(0, m_nSli - 1), MyBasic::Range(0, m_nRow - 1), MyBasic::Range(0, m_nCol - 1)), val);
  }
  ...
  void copyFrom(const Data3D<T> & src, const MyBasic::Range3D & rsrc, const MyBasic::Range3D & rdst)
  {
  ...
  //copy data using rects
  template<typename T2>
  void copyFrom(const Data3D<T2> & src, const MyBasic::Range3D & rsrc, const MyBasic::Range3D & rdst)
  {
    smart_assert(inRange(rdst), "The box is outof range");
    smart_assert(src.inRange(rsrc), "The destination box is outof range");
    smart_assert(MyBasic::Size3D::isEqual(rsrc.getSize(), rdst.getSize()), "The copy ranges should be equal");
  ...
  //////////////////////////////////////////////////////////////////////////////////
  inline bool inRange(const MyBasic::Index3D & p) const
  {
    return inRange(p.sli, p.row, p.col);
  }
  inline bool inRange(const MyBasic::Range3D & r) const
  {
    return inRange(MyBasic::Index3D(r.start.sli, r.start.row, r.start.col)) && inRange(MyBasic::Index3D(r.end.sli, r.end.row, r.end.col));
  }
  ...
  {
    return MyBasic::Size3D(m_nSli, m_nRow, m_nCol);
  }
