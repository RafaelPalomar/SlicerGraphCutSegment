  ...
  :image(_image), mask(_mask), mid_slice(_mid_slice)
{
  ...
  bestLambda = std::vector<double>(_image.getLength());
...
void AdaptiveSegment3D::setAreaRatio(const std::vector<double> & min_ratio, const std::vector<double> & max_ratio)
{
...
{
  std::cout << "Search parameters on " << ind << " slice." << std::endl;
  ...
  */
  ...
  if (cfg.smoothterm)
  {
    std::pair<Data2D<double>, Data2D<double> > smoothCostR2D = seg.smoothCostR;  //pair cap rev_cap
    smoothCostR.first.setSlice(ind, smoothCostR2D.first);
    smoothCostR.second.setSlice(ind, smoothCostR2D.second);
    std::pair<Data2D<double>, Data2D<double> > smoothCostC2D = seg.smoothCostC;  //pair cap rev_cap
...
// cout<<" before compute star shape"<<endl;
  ...
  std::cout << "Done add dataterm  1" << std::endl;
  m_pGraph->addDataCost(extraDataCost.id, extraDataCost.fgcost, extraDataCost.bkcost);
  std::cout << "Done add extra dataterm  2" << std::endl;
  if (cfg.smoothterm)
  {
  ...
  std::cout << "Done add smoothterm  3" << std::endl;
  std::cout << extraSmoothCost.id1.size() << ";" << extraSmoothCost.id2.size()
    << extraSmoothCost.cap.size() << extraSmoothCost.rev_cap.size() << std::endl;
  //	return;
...
void AdaptiveSegment3D::resegment(const Data3D<LABEL> * new_seeds)
{
  ...
  if (new_seeds == NULL)
  {
    std::cout << "no new user seeds, nothing is done" << std::endl;
    return;
  }
