...
void bresenham(int x1, int y1, int x2, int y2, std::vector<int>& py)
{
...
void addNextConstraintPixels2D(SparseNLink & slc, int dims[], int seed[], int point[],
  std::vector<unsigned char> & markings)

{
  ...
  int numPix = x2 - x1 + 1;
  std::vector<std::vector<int> > pt(2, std::vector<int>(numPix, 0));
  std::vector<int> pixels(numPix);  //pixel ID for current dimension
...
{ //vector pixels is the pixelID for pixels on this slice
 ...
 //  cv::RNG rng(12345);
  ...
  std::vector<unsigned char> markings(num_pixels, 0);
  std::vector<int> pixelOrder(num_pixels);
...

{
  ...
  int numPix = x2 - x1 + 1;
  std::vector<std::vector<int> > pt(3, std::vector<int>(numPix, 0));
